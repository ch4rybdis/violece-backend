...
use App\Services\Matching\CompatibilityScoringService;
        ...
        $questionCount = PsychologyQuestion::count();
        ...
        $this->info('🎭 Creating test users with psychological profiles...');
            ...
            $user1 = User::factory()->create([
                'first_name' => 'Alice (Secure & Agreeable)',
                'uuid' => \Illuminate\Support\Str::uuid(),
            ]);

            $user2 = User::factory()->create([ 'first_name' =>  'Bob (Anxious & Creative)',
                'uuid' => \Illuminate\Support\Str::uuid(),]);
        $user3 = User::factory()->create([ 'first_name' => 'Charlie (Avoidant & Organized)',
            'uuid' => \Illuminate\Support\Str::uuid(),]);
...
private function simulateQuestionnaireResponse(User $user, array $traits)
{
            ...
            $questions = PsychologyQuestion::with('options')->get();
            $responses = [];
        ...
        $score -= ($weights['agreeableness'] ?? 0) * 2;
            ...
            if ($traits['secure_attachment'] ?? false) {
                $score += ($weights['secure'] ?? 0) * 3;
            }
            if ($traits['anxious_attachment'] ?? false) {
                $score += ($weights['anxious'] ?? 0) * 3;
            }
            if ($traits['avoidant_attachment'] ?? false) {
                $score += ($weights['avoidant'] ?? 0) * 3;
            }
